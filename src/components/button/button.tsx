...
// Button props
interface ButtonWrapperProps extends PrimitveButtonProps {
...
const Button: Component<ButtonWrapperProps> = (props) => {
    ...
    return (
        <Dynamic component={local.asChild || 'button'} ref={local.ref} class={cn("flex items-center justify-center gap-3  outline-none cursor-pointer p-4 rounded-sm text-[1rem] w-full mx-auto", local?.class)} {...others}>
            {local.children}
        </Dynamic>
    );
};
